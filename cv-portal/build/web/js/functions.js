...
$(document).ready(function () {
	...
	var menuEhIgualUrl = function menuEhIgualUrl(href, url) {
		url = url.split('?')[0];
		return url == href || url == href + "index.php" || url + "index" == href || url + "index" == href + "index.php";
	};

	/* VERIFICA MENU ABERTO */
	var abreMenu = function abreMenu(url) {
		var found = false;
		var isEvento = $('#navigation').hasClass("evento");
		var idInput = undefined;
	...
	if (width > 768) {
		var finalUrl = url.replace(domain, "").replace("index.php", "").replace(".php", "");
		var found = abreMenu(finalUrl);
		if (!found) {
			var urlWithoutEnd = finalUrl.substr(0, finalUrl.lastIndexOf("\/") + 1);
			var _found = abreMenu(urlWithoutEnd);
		}
	}
